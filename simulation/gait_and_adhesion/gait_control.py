...
def cup_link_name(leg):
    # '{leg}-l6' no longer exists as an independent link in Gazebo -- since
    # j4/j5 are type="fixed", URDF->SDF conversion lumps l5 and l6 into l4
    # (standard behavior for fixed-joint chains, confirmed directly via
    # gz topic contact data: body names show "...l4_fixed_joint_lump__l6...").
    # l4/l5/l6 are now one rigid body regardless, so attaching to l4
    # achieves exactly the same physical result l6 would have.
    return f'{leg}-l4'
...
CYCLE_PERIOD = 4.0         # seconds for one full stance+swing cycle
...
class SuctionCupManager:
    ...
    def __init__(self, node: Node):
        ...
        # Separate callback group from the control-loop timer -- a
        # MultiThreadedExecutor alone does NOT parallelize callbacks that
        # share the node's default MutuallyExclusiveCallbackGroup (a
        # well-documented rclpy behavior: everything in one group still
        # runs one-at-a-time regardless of executor thread count).
        # Without this, the 20Hz timer can starve these service response
        # callbacks from ever running, even after the server has already
        # replied -- confirmed by testing: cups were physically, genuinely
        # attached, but client-side state never advanced past ATTACHING.
        self._cup_callback_group = ReentrantCallbackGroup()

        for leg in LEG_IDS:
            self._attach_clients[leg] = node.create_client(
                AttachLink, f'/leg{leg}/ATTACHLINK', callback_group=self._cup_callback_group
            )
            self._detach_clients[leg] = node.create_client(
                DetachLink, f'/leg{leg}/DETACHLINK', callback_group=self._cup_callback_group
            )
    ...
    def _on_done(self, leg, on, future):
        ...
        try:
            ...
            if not result.success:
                self.node.get_logger().warn(
                    f'Leg {leg}: {"attach" if on else "detach"} failed -- {result.message}'
                )
...
def main(args=None):
    ...
    # MultiThreadedExecutor, not the default single-threaded rclpy.spin():
    # this node has both a 20Hz control-loop timer AND async suction cup
    # service clients. With a single thread, the timer firing constantly
    # can starve the service response callbacks from ever running, even
    # after the server has already replied -- confirmed by testing: cups
    # were physically, genuinely attached (the C++ side succeeded), but
    # the client-side state never advanced past ATTACHING, and detach
    # calls suffered the same fate, leaving legs permanently welded
