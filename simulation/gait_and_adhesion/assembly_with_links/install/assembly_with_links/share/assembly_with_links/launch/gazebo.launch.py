...
def generate_launch_description():
    ...
    # NOTE: this used to set GAZEBO_PLUGIN_PATH to point at a
    # ros2_linkattacher install. That was specific to IFRA_LinkAttacher,
    # which we dropped entirely (see assembly_with_links.urdf) after
    # confirming it only ever supports one active attachment at a time.
    # gazebo_ros_vacuum_gripper (what we use now) is found without any
    # override -- your own /leg{N}/switch and /leg{N}/grasping services
    # already confirmed working -- so this was just stale leftover
    # pointing at a package path that's no longer relevant. Removed
    # rather than left in place to avoid it being misread as still
